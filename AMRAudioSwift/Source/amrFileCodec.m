...
NSData *processAndroid3GP(NSData *data) {
    u32 size = 0; 
    u32 type = 0;
    u32 boxSize = 0;
...
#pragma mark - Decode
const int roundNumber(const double x) {
	return((int)(x + 0.5));
} 
...
int caclAMRFrameSize(unsigned char frameHeader) {
	...
	frameSize = roundNumber((double)temp2 + 0.5);
	return frameSize;
}
...
NSData *EncodePCMToAMR(char *data, int maxLen,int nChannels, int nBitsPerSample, enum Mode nBitRateMode) {
    char *oldBuf = data;
    /* input speech vector */
	short speech[160];
	...
	/* requested mode */
	enum Mode req_mode = nBitRateMode;
	int dtx = 0;
...
NSData *EncodeWAVEToAMR(NSData *data, int nChannels, int nBitsPerSample, enum Mode nBitrateMode) {
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentPath = [paths objectAtIndex:0];
    ...
    return EncodePCMToAMR(buf, maxLen - nPos, nChannels, nBitsPerSample, nBitrateMode);
}
